...
파싱된 문서 텍스트를 검색용 청크로 분할한다.

토큰 카운팅은 간단한 추정(공백 단위 단어 × 1.3)을 사용한다.
Bedrock Titan embedding 의 입력 한도는 8192 토큰이므로
CHUNK_SIZE_TOKENS=1000 은 충분히 안전한 설정이다.
"""
...
def estimate_tokens(text: str) -> int:
    ...
    정확한 토크나이저를 사용하지 않아도 청킹/가드 용도로는 충분하다.
    한국어는 단어당 ~1.5토큰, 영어는 ~1.3토큰 수준이므로 평균 1.3 사용.
    """
    if not text:
        return 0
    words = text.split()
